...
trim_file_to_payload() {
  ...
  if ! collapse_repeated_log_blocks "$source_file" "$collapsed_file"; then
    tail -c "$CI_AGENT_PAYLOAD_TAIL_BYTES" "$source_file" >"$target_file" 2>/dev/null || cp "$source_file" "$target_file"
    rm -f "$collapsed_file"
    return 0
  fi

  tail -c "$CI_AGENT_PAYLOAD_TAIL_BYTES" "$collapsed_file" >"$target_file" 2>/dev/null || cp "$collapsed_file" "$target_file"
  rm -f "$collapsed_file"
}

collapse_repeated_log_blocks() {
  local source_file="$1"
  local target_file="$2"

  awk -v max_lines="${CI_AGENT_COLLAPSE_MAX_LINES:-100000}" '
    BEGIN {
      if ((max_lines + 0) < 1) {
        max_lines = 100000
      }
    ...
    END {
      ...
      while (i <= n) {
        ...
        for (block_len = 1; block_len <= max_len; block_len++) {
          count = 1
          next_start = i + block_len

          while ((next_start + block_len - 1) <= n) {
            matches = 1
            for (j = 0; j < block_len; j++) {
              if (lines[i + j] != lines[next_start + j]) {
                matches = 0
                break
              }
            }
        ...
        if (best_count > 1) {
          if (best_len == 1) {
            print lines[i] " (repeated " best_count "x)"
          } else {
            for (j = 0; j < best_len; j++) {
              print lines[i + j]
            }
            print "[previous " best_len "-line block repeated " best_count "x]"
          }
  ...
  while true; do
    ...
    if http_code="$(curl "${curl_args[@]}")"; then
      if [[ "$http_code" == "401" ]] && (( refreshed_oidc_token == 0 )); then
        if can_refresh_github_oidc_token; then
        ...
        log "ci-agent repair returned HTTP 401"
...
build_payload() {
  ...
  jq -n \
    ...
    --arg repository_id "${CI_AGENT_REPOSITORY_ID:-}" \
...
require_env CI_AGENT_REPOSITORY_ID
