...
export class AppController {
  ...
  async renderTemplate(@Body() raw): Promise<string> {
    if (typeof raw === 'string' || Buffer.isBuffer(raw)) {
      ...
      const safeTemplate = dotT.compile('{{=it.userInput}}');
      const res = safeTemplate({ userInput: text });
  ...
  getSecrets(): Record<string, string> {
    const secrets = {
      ...
      google_b64: process.env.GOOGLE_B64_TOKEN || '',
      google_oauth: process.env.GOOGLE_OAUTH_CLIENT_ID || '',
      google_oauth_token: process.env.GOOGLE_OAUTH_ACCESS_TOKEN || '',
