...
export class PartnersController {
  ...
  private isValidInput(input: string): boolean {
    // Basic validation to check for invalid characters
    const invalidChars = /['"&<>]/;
    return !invalidChars.test(input);
  }

  private isValidXPath(xpath: string): boolean {
    // Basic validation to check for invalid XPath patterns
    const invalidPatterns = /['"&<>]|(or|and|=|!=|<=|>=|<|>)/;
    return !invalidPatterns.test(xpath);
  }
}